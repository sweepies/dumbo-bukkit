	...
	private void createConfig() {
		try {
			...
			if (!file.exists()) {
				...
				InputStream in = new URL("https://raw.githubusercontent.com/sweepyoface/dumbo/master/quotes.yml")
						.openStream();
			...
			if (args.length == 0) {
				if (player.hasPermission("dumbo.quote") || player.isOp()) {
					Bukkit.getServer().broadcastMessage(
							ChatColor.translateAlternateColorCodes('&', this.getConfig().getString("color"))
									+ randomQuote());
				}
